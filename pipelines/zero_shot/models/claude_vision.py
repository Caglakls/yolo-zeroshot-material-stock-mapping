...
CHECKPOINT_NAME = "claude_detailed_checkpoint.json"
...
def _result_from_raw(raw_text: str) -> ClaudeResult:
...
def query_claude_single(
    client: anthropic.Anthropic,
    image_path: str,
    model: str,
) -> ClaudeResult:
    """Send one image to Claude Vision API, parse response."""
...
def _checkpoint_entry_to_result(entry: dict) -> ClaudeResult:
    return ClaudeResult(
...
def _result_to_checkpoint_entry(result: ClaudeResult) -> dict:
    return {
...
def run_claude_batch(
    ...
    """Process all images through Claude Vision with rate limiting and checkpointing."""
    ...
    for i, record in enumerate(tqdm(remaining, desc="Claude inference")):
        start_time = time.time()
        try:
            result = query_claude_single(client, record.image_path, model)
        except Exception as e:
            logger.error("Claude failed for %s: %s", record.image_id, e)
            result = ClaudeResult(
    ...
    save_checkpoint(checkpoint_path, checkpoint_data)
    logger.info("Claude: processed %d images", len(results))
    return results
